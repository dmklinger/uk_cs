		...
		if self.form_type not in ('verb', 'adj'):
			...
			for form in [x for x in self.forms if x != 'inf']:
				...
				if len(info) == 2:
					...
					new_forms['past'][info[0]][info[2]] = self.forms[form]
...
class Usage:
		...
		for info in self.info:
			for word in info.split():
				...
				if word in ('imperfective', 'impf'):
					aspect.add('недоконаний')
				if word in ('pf', 'perfective'):
					aspect.add('доконаний')
		new_info = []
		if len(gender) > 0:
	...
	def get_forms(self, final_forms=False):
		...
		for form_id in self.forms:
			...
			if final_forms:
				forms = self.forms[form_id].get_final_forms(self.word)
	...
	def get_form_words(self):
		...
		for forms in self.get_forms().values():
			for f in forms:
				f = f.replace('́', '')
				f = re.sub(r"[^ěščřžýáíéóúůďťňĎŇŤŠČŘŽÝÁÍÉÚŮĚÓa-zA-z']+", ' ', f).strip().split()
				results += f
		return results
...
class Word:
	...
	def add_frequencies(self, frequencies):
		for _, usage in self.usages.items():
			if frequencies:
				usage.add_frequency(frequencies)
			else:
